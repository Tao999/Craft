...
public class CraftView extends Group {
	...
	public CraftView(Scene scene) {
		...
		this.getChildren()
				.add(this.drawCraft(0, 0, r.getWidth(), r.getHeight() / 2));
		this.getChildren().add(this.drawInventory(0, r.getHeight() / 2,
				r.getWidth(), r.getHeight() / 2 + r.getHeight() / 2));
	...
	private Group drawCraft(double x, double y, double width, double height) {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				Rectangle r = new Rectangle(i * (slotSize + slotMargin),
						j * (slotSize + slotMargin), slotSize, slotSize);
	...
	private Group drawInventory(double x, double y, double width,
			double height) {
		Group g1 = new Group();
